...
library RentLib {
        ...
        returns (uint256, bool)
    {
        ...
        require(ERC721.exists(rentParams._assetId), "_assetId not found");
        Marketplace.Asset memory asset = ms.assets[rentParams._assetId];
        require(
            asset.status == Marketplace.AssetStatus.Listed,
            "_assetId not listed"
        );
        require(
        ...
        (uint256 rentFee, uint256 protocolFee) = Fee.distributeFees(
            rentParams._assetId,
            asset.paymentToken,
            rentPayment
        );
        uint256 rentId = Marketplace.addRent(
            rentParams._assetId,
            msg.sender,
            rentStart,
            rentEnd
