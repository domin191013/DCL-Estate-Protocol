...
library Diamond {
    bytes32 constant DIAMOND_STORAGE_POSITION =
        keccak256("com.dcl.estate.storage");
    ...
    function addFunctions(
        ...
        require(
            ...
            "DiamondCut: No selectors in facet to cut"
        );
        ...
        internal
    {
        enforceHasContractCode(
            _facetAddress,
            "DiamondCut: New facet has no code"
        );
        ...
        internal
    {
        if (_init == address(0)) {
            require(
                _calldata.length == 0,
                "DiamondCut: _init is address(0) but_calldata is not empty"
            );
        } else {
            require(
                _calldata.length > 0,
                "DiamondCut: _calldata is empty but _init is not address(0)"
            );
