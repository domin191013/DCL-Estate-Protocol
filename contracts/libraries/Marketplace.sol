...
library Marketplace {
    ...
    bytes32 constant MARKETPLACE_STORAGE_POSITION =
        keccak256("com.dcl.estate.marketplace");
    ...
    function setRegistry(
        ...
        Marketplace.MetaverseRegistry storage mr = marketplaceStorage()
    ...
    function addRent(
        ...
        Marketplace.MarketplaceStorage storage ms = marketplaceStorage();
        uint256 newRentId = ms.assets[_assetId].totalRents + 1;

        ms.assets[_assetId].totalRents = newRentId;
        ms.rents[_assetId][newRentId] = Marketplace.Rent({
            renter: _renter,
            start: _start,
            end: _end
        });
